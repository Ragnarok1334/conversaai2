...
BEGIN
  ...
  IF v_plan = 'starter' THEN
    v_assistants_limit := 1;
    v_messages_limit := 500;
  ELSIF v_plan = 'pro' THEN
    v_assistants_limit := 3;
    v_messages_limit := 2500;
  ELSIF v_plan = 'growth' THEN
    v_assistants_limit := 8;
    v_messages_limit := 8000;
  ELSIF v_plan = 'business' THEN
    v_assistants_limit := 20;
    v_messages_limit := 20000;
  ELSE
    RETURN jsonb_build_object('success', false, 'code', 'invalid_paid_plan');
  END IF;

  UPDATE public.billing_payments
  SET status = 'paid',
      raw_response = p_flow_status,
      metadata = COALESCE(metadata, '{}'::jsonb) ||
        CASE WHEN status = 'cancelled' THEN jsonb_build_object('recoveredFromCancelled', true) ELSE '{}'::jsonb END,
      updated_at = v_now
  ...
  ELSE
    INSERT INTO public.subscriptions (
      ...
      grace_ends_at, cancel_at_period_end, cancelled_at, cancellation_reason
    ) VALUES (
      v_payment.user_id, v_plan, 'active', v_assistants_limit, v_messages_limit,
      0, v_now, v_now + interval '30 days',
      v_now + interval '32 days', false, null, null
    );
  END IF;
