...
begin
  ...
  -- Serialize assistant creation per user so concurrent requests cannot bypass the limit.
  perform pg_advisory_xact_lock(hashtextextended(new.user_id::text, 0));

  select s.plan, s.assistants_limit, s.current_period_end, s.grace_ends_at,
  ...
  if v_plan = 'trial' then
    select p.trial_ends_at into v_trial_end
    ...
    elsif coalesce(v_cancel_at_period_end, false) = false
      and v_grace_end is not null and now() <= v_grace_end then
      if v_plan = 'enterprise' then
        v_limit := null;
      elsif v_limit is null then
        v_limit := case v_plan
  ...
  if v_limit is not null and v_count >= v_limit then
    raise exception using
      errcode = 'P0001',
      message = 'assistant_limit_reached';
  end if;
...
create trigger enforce_assistant_limit_before_insert
