...
AS $$
BEGIN
  -- Server-side operations (service role / trusted DB jobs) may manage trial state.
  -- A normal authenticated client may only update profile fields, never trial state.
...
BEGIN
  ...
  IF NOT FOUND OR v_trial_used THEN
    RETURN false;
  END IF;
