...
BEGIN
  ...
  IF p_flow_status->>'commerceOrder' IS NULL OR v_payment.flow_order IS NULL
  ...
  IF FOUND THEN
    UPDATE public.subscriptions SET plan=v_plan,status='active',assistants_limit=v_assistants_limit,messages_limit=v_messages_limit,
      current_messages_used=0,current_period_start=v_now,current_period_end=v_now+interval '30 days',grace_ends_at=v_now+interval '32 days',
      cancel_at_period_end=false,cancelled_at=null,cancellation_reason=null,updated_at=v_now WHERE id=v_subscription.id;
  ELSE
    INSERT INTO public.subscriptions(user_id,plan,status,assistants_limit,messages_limit,current_messages_used,current_period_start,current_period_end,grace_ends_at,cancel_at_period_end,cancelled_at,cancellation_reason)
    VALUES(v_payment.user_id,v_plan,'active',v_assistants_limit,v_messages_limit,0,v_now,v_now+interval '30 days',v_now+interval '32 days',false,null,null);
  END IF;
