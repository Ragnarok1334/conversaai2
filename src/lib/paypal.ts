...
function getApiBase(): string {
  return getMode() === 'live' ? LIVE_API : SANDBOX_API;
}
...
export async function createPayPalOrder(input: {
  ...
  const response = await authorizedFetch('/v2/checkout/orders', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'PayPal-Request-Id': input.referenceId },
    body: JSON.stringify({
      intent: 'CAPTURE',
      purchase_units: [{
        reference_id: input.plan,
        description: `ConversaAI ${input.plan} - Suscripción mensual`,
        amount: { currency_code: 'USD', value: (input.amountCents / 100).toFixed(2) },
      }],
      application_context: {
        brand_name: 'ConversaAI',
        user_action: 'PAY_NOW',
...
export async function capturePayPalOrder(orderId: string): Promise<PayPalOrderResult> {
  if (!/^[A-Z0-9-]{8,64}$/i.test(orderId)) throw new Error('ID de orden PayPal inválido.');
  const response = await authorizedFetch(`/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'PayPal-Request-Id': orderId },
    body: '{}',
  });
  const data = await parsePayPalJson(response);
  if (!data || typeof data !== 'object') throw new Error('Respuesta PayPal inválida.');
...
export async function verifyPayPalWebhook(input: {
  ...
  const response = await authorizedFetch('/v1/notifications/verify-webhook-signature', {
    method: 'POST',
