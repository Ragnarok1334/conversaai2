...
export function Sidebar({ onNavClick }: SidebarProps) {
  ...
  return (
    <aside className="dashboard-sidebar flex flex-col w-full lg:w-64 min-h-screen backdrop-blur-2xl lg:sticky lg:top-0 h-screen overflow-y-auto">
      {/* Logo */}
      <div className="px-6 py-6 border-b border-white/[0.06]">
        <Link href="/" className="flex items-center gap-3 group" onClick={onNavClick}>
          ...
          <div>
            <span className="dashboard-brand-name text-lg font-bold tracking-tight block">ConversaAI</span>
            <span className="text-[10px] uppercase font-bold tracking-widest text-brand-violet">Panel SaaS</span>
          </div>
        </Link>
      </div>
            ...
            <Link key={item.href} href={item.href} onClick={onNavClick}>
              <motion.div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all relative overflow-hidden group ${
                  isActive
                    ? 'dashboard-nav-active shadow-sm'
                    : 'dashboard-nav-idle'
                }`}
              >
                {isActive && (
                  <div className="dashboard-nav-active-bg absolute inset-0 rounded-xl" />
                )}
