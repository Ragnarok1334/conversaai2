...
export async function GET() {
  try {
    ...
    const [
      ...
      supabase.from('assistants').select('id, assistant_name, business_name, channel, status, created_at, tone, assistant_channels:assistant_channels!assistant_channels_assistant_user_fkey(channel, is_enabled)').eq('user_id', user.id).order('created_at', { ascending: false }).limit(5),
