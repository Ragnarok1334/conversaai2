...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase
      .from('assistants').select(SELECT_FIELDS).eq('id', id).eq('user_id', user.id).single()
    ...
    if (conversations.error || leads.error) {
      console.error('[GET /api/assistants/[id]] activity query failed')
    ...
    const timestamps = [
      ...
      ...convData.map(x => x.created_at ? new Date(x.created_at).getTime() : 0),
      ...leadData.map(x => x.created_at ? new Date(x.created_at).getTime() : 0),
    ...
    const health = calculateAssistantHealth(
      ...
      { conversations: convData.length, leads: leadData.length }
    )

    return NextResponse.json({
      assistant: {
        ...data,
        conversationsCount: convData.length,
        leadsCount: leadData.length,
        lastActivityAt: new Date(timestamps.length ? Math.max(...timestamps) : Date.now()).toISOString(),
        health,
      },
    })
  } catch (error) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body) || serializedSize(body) > MAX_BODY_BYTES) {
      return NextResponse.json({ error: 'Cuerpo de solicitud inválido o demasiado grande.' }, { status: 400 })
    }
    ...
    for (const [from, to] of Object.entries(aliases)) {
      if (from in input && to in input) return NextResponse.json({ error: `No envíes '${from}' y '${to}' al mismo tiempo.` }, { status: 400 })
    }

    const limits: Record<string, number> = {
      assistant_name: 100, business_name: 100, business_type: 100, instructions: 2_000,
      main_goal: 500, fallback_message: 500, faqs: 10_000, services: 10_000, schedule: 5_000, language: 10,
    }
    const allowed = new Set([
      'assistant_name', 'business_name', 'business_type', 'instructions', 'behavior', 'channel', 'tone',
      'main_goal', 'fallback_message', 'status', 'knowledge_blocks', 'faqs', 'services', 'schedule', 'language', 'widget_config',
    ])
    ...
    for (const [key, value] of Object.entries(input)) {
      ...
      if (target in limits) {
        ...
        val = val.trim()
        if (val.length > limits[target]) return NextResponse.json({ error: `El campo ${target} excede su límite.` }, { status: 400 })
      }
      ...
      if (target === 'behavior') {
        ...
        if (rules !== undefined) {
          ...
          for (const [rule, ruleValue] of Object.entries(rules as Record<string, unknown>)) {
            if (!allowedRules.includes(rule) || typeof ruleValue !== 'boolean') return NextResponse.json({ error: `La regla '${rule}' no es válida.` }, { status: 400 })
          }
        }
      }
    ...
    if (updates.instructions !== undefined || updates.knowledge_blocks !== undefined) {
      ...
      const validBlock = Array.isArray(blocks) && blocks.some((b: any) => b?.is_active === true && typeof b?.content === 'string' && b.content.trim().length >= 80)
