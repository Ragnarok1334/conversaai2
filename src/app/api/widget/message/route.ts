...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!message || message.length > MAX_MESSAGE_LENGTH) return NextResponse.json({ error: 'Mensaje inválido o demasiado largo.' }, { status: 400, headers: corsHeaders })
    if (conversationId && !UUID_RE.test(conversationId)) return NextResponse.json({ error: 'conversationId inválido.' }, { status: 400, headers: corsHeaders })
    if (visitorId.length > MAX_VISITOR_ID_LENGTH) return NextResponse.json({ error: 'visitorId inválido.' }, { status: 400, headers: corsHeaders })
    ...
    if (assistantError || !assistant) return NextResponse.json({ error: 'Asistente no encontrado.' }, { status: 404, headers: corsHeaders })
    if (assistant.status !== 'active') return NextResponse.json({ error: 'El asistente no está activo.' }, { status: 403, headers: corsHeaders })
    ...
    if (!await checkRateLimit(`widget-ip-${assistantId}-${ip}`, 'widget-message-ip-minute', 60, 60)) {
    ...
    if (!await checkRateLimit(`widget-vis-${assistantId}-${visId}`, 'widget-message-vis-minute', 20, 60)) {
    ...
    if (subRes.error || profileRes.error) return NextResponse.json({ error: 'No se pudo validar el estado del asistente.' }, { status: 503, headers: corsHeaders })

    const effectiveStatus = getEffectiveSubscriptionStatus(subRes.data, profileRes.data)
    if (['free', 'expired', 'cancelled'].includes(effectiveStatus)) return NextResponse.json({ error: 'El plan del asistente no está activo. Renueva tu plan o activa tu prueba para continuar.' }, { status: 403, headers: corsHeaders })
    ...
    if (!await consumeMessageCredit(ownerId, effectiveLimit)) {
    ...
    if (currentConversationId) {
      const { data: conv } = await supabaseAdmin.from('conversations')
        ...
        .eq('id', currentConversationId).eq('assistant_id', assistantId).eq('user_id', ownerId)
        .select('id').maybeSingle()
      if (!conv) currentConversationId = null
    }

    if (!currentConversationId) {
      const { data: conv, error: convError } = await supabaseAdmin.from('conversations')
        ...
        .select('id').single()
      if (convError || !conv) throw new Error('conversation_create_failed')
    ...
    if (userMessageError) throw new Error('user_message_save_failed')
    ...
    if (assistantMessageError) throw new Error('assistant_message_save_failed')
    ...
    if (extractedEmail || extractedPhone || extractedName) {
      ...
      if (existingLead) {
        ...
        if (newLead) await supabaseAdmin.from('notifications').insert({ user_id: ownerId, title: 'Nuevo lead capturado', message: 'Un visitante dejó sus datos desde Web Chat.', type: 'lead', metadata: { leadId: newLead.id, assistantId, conversationId: currentConversationId } })
      }
    }
    ...
    if (creditConsumed && creditOwnerId) {
      ...
      if (!refunded) console.error('[POST /api/widget/message] Failed to refund consumed credit')
    }
